...
The two identical preamble symbols serve a dual purpose: the first gives the
timing reference and the channel estimate; the pair enables CFO estimation.
...
\begin{tabular}{lll}
\toprule
\textbf{Subcarriers} & \textbf{Indices} & \textbf{Count} \\
\midrule
DC + upper guard & 0, 27--37 & 12 \\
Data & all others not in pilots/null & 48 \\
Pilots & 7, 21, 43, 57 & 4 \\
\midrule
...
The \texttt{NULL\_IDX} set mirrors 802.11a/g (Wi-Fi) guard subcarrier
placement, so the spectrum is compatible with standard spectral masks.
...
\begin{enumerate}
  \item \textbf{Constant envelope}: $|x[n]| = 1$ for all $n$.
        No PAPR penalty from the preamble.
  \item \textbf{Flat spectrum}: the IFFT of a frequency-domain ZC sequence has
        nearly constant envelope in time domain too, giving low PAPR.
  \item \textbf{Sharp autocorrelation}: $|R_{xx}[\tau]| = 0$ for
        $\tau \neq 0 \pmod{N_u}$, so the cross-correlation with the reference
        gives a single clean spike, enabling precise timing.
...
Pilots (4 subcarriers per symbol) give only 4 channel samples per data symbol,
which is too sparse for interpolation in a frequency-selective channel.
The preamble gives 52 samples in one shot before any data arrives, at no
throughput cost since the preamble is overhead anyway.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{EVM: How Healthy Is the Link?}

Error Vector Magnitude (EVM) is measured on the 4 pilot subcarriers of the
preamble.  After LS estimation, $\hat{H}[k_p]/X_{\text{ZC}}[k_p]$ should equal
1 for a perfect channel.  The deviation is the error vector:
\begin{equation}
  \text{EVM} = 10\log_{10}\!\left(\frac{1}{4}\sum_{p}
    \left|\hat{H}[k_p] - 1\right|^2\right) \text{ dB}
\end{equation}

Typical values observed on the bench:
\begin{center}
\begin{tabular}{ll}
\toprule
\textbf{EVM (dB)} & \textbf{Interpretation} \\
\midrule
$< -25$ & Excellent (BER$\approx$0 for QAM64) \\
$-20$ to $-10$ & Good (QPSK and QAM16 work reliably) \\
$-5$ to $0$ & Marginal (QPSK may still work) \\
$> 0$ & Poor (likely sync failure or deep fade) \\
\bottomrule
\end{tabular}
\end{center}

In the OTA test reported here, EVM was 11--15\,dB (i.e.\ $-11$ to $-15$\,dB),
giving BER~$= 0$ for QPSK.
...
\begin{enumerate}
  ...
  \item \textbf{Add a length field.}  The current frame has no header telling
    ...
    data symbol (with a known pilot scheme) so the RX can set \texttt{max\_syms}
    automatically.
\end{enumerate}
