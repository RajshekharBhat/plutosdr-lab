...
\usepackage{mdframed}
\newmdenv[linewidth=1pt, linecolor=black!40, backgroundcolor=blue!4,
          innertopmargin=8pt, innerbottommargin=8pt,
          frametitle={\small\textbf{For the curious ---}},
          frametitlealignment=\raggedright]{curiositybox}
...
\begin{equation}
  x[n] = \begin{cases}
    \exp\!\left(-j\pi q\,\dfrac{n(n+1)}{N_u}\right) & N_u \text{ odd} \\[8pt]
    \exp\!\left(-j\pi q\,\dfrac{n^2}{N_u}\right)     & N_u \text{ even}
  \end{cases}
  \quad n = 0,1,\ldots,N_u-1
\end{equation}

Here $N_u = 52$ (number of active subcarriers, \textbf{even}), $q = 1$.
In LTE/5G, ZC sequences always use odd prime lengths (e.g.\ 839 in LTE PSS)
to guarantee the CAZAC property unambiguously.  For our $N_u=52$, $q=1$
case, $\gcd(q, N_u)=1$ ensures perfect cyclic autocorrelation under the
even-$N$ formula.
...
ZC_ACTIVE = _zc(52)          # 52-point ZC, even N → uses n^2 form
...
% ── Barker note ──────────────────────────────────────────────────────────────
\subsection{Aside: Why Not a Barker Sequence?}

In a single-carrier system (e.g.\ in Digital Communications courses), the
standard choice for frame synchronisation is a \textbf{Barker sequence} ---
a binary $(\pm 1)$ sequence with the best possible \emph{aperiodic}
autocorrelation:
\begin{equation}
  |R_{\text{Barker}}[\tau]| \leq 1 \quad \text{for all } \tau \neq 0
\end{equation}

The known Barker sequences are:

\begin{center}
\begin{tabular}{clc}
\toprule
\textbf{Length} & \textbf{Sequence} & \textbf{Peak/sidelobe} \\
\midrule
2  & $+{+}-$                                     & 2 \\
3  & $+{+}-$                                     & 3 \\
4  & $+{+}-{+}$                                  & 4 \\
...
No Barker sequence of length $> 13$ is known to exist.

\paragraph{Why Barker works for frame sync in single-carrier systems.}
In a single-carrier link, the preamble has one job: mark the frame boundary.
A length-13 Barker sequence gives a correlation peak of 13 with sidelobes
$\leq 1$ (ratio 13:1), which is sufficient to detect the frame start
reliably at moderate SNR.

\paragraph{Why we use ZC instead for OFDM.}
Our preamble has \emph{two} jobs: timing \emph{and} channel estimation over
all 52 active subcarriers.  A Barker sequence has only 13 values, so mapping
it to frequency gives at most 13 known subcarrier values --- too few for
accurate channel estimation across 52 subcarriers.

ZC gives 52 known values simultaneously, one per active subcarrier, with
flat power across the band so every subcarrier is estimated with equal
accuracy.

\paragraph{If we separated the two roles} (as 802.11a does), Barker would be
an excellent choice for the timing-only part: binary, no complex arithmetic,
mathematically proven sidelobe bound, and familiar from single-carrier
courses.  The separate channel estimation symbol could then be a ZC or any
other flat-spectrum known sequence.

\begin{curiositybox}
\textbf{Is ZC uniquely good for channel estimation?}

For LS channel estimation the MSE at subcarrier $k$ is
$\sigma^2/|X[k]|^2$.  The only requirement on the training sequence is
\textbf{equal amplitude on all pilot subcarriers} ($|X[k]| = \text{const}$)
--- this minimises and equalises the noise amplification.
ZC satisfies this ($|x[n]|=1$), but so does any other constant-amplitude
sequence: all-ones, random BPSK ($\pm 1$), or even Barker extended to the
required length.  ZC is \emph{not} theoretically superior to these for
estimation quality alone.

In practice, what matters far more than the sequence values is
\textbf{pilot placement}: pilots must be uniformly spaced in frequency
(spacing $\leq F_s/(N\,\tau_{\max})$) and dense enough in time to track
the channel coherence time $T_c$.

Real standards reflect this.  Wi-Fi 802.11a uses a predefined $\pm1/\pm j$
sequence (chosen by exhaustive search for low PAPR, not by formula) for its
Long Training Sequence and gets the same LS estimation quality as ZC.
LTE uses ZC-based DMRS pilots on a \emph{comb} pattern (every 6th
subcarrier, updated every 7 symbols) and relies on interpolation between
pilots --- the ZC choice there is for its flexibility in length and its
dual role in synchronisation, not for any estimation advantage over other
unit-amplitude sequences.

\medskip
\textbf{Then why does 5G use Gold sequences instead of ZC for DMRS?}

Equal amplitude is necessary but not sufficient in a multi-cell system.
When neighbouring cells transmit pilots simultaneously, if they use the
same sequence your channel estimate becomes
$\hat{H} = H_{\text{desired}} + H_{\text{interferer}}$
--- the interfering channel leaks in directly.  This is called
\textbf{pilot contamination}.

Gold sequences (based on a degree-31 generator polynomial) provide
$\approx 2^{31}$ distinct sequences, each with low cross-correlation to
all others.  The seed encodes cell ID, UE ID, slot, and symbol number,
so every user in every cell in the world gets a unique pilot that looks
like noise to everyone else.  ZC of length $N$ gives at most $N-1$
distinct sequences --- sufficient for one cell, but not for dense
deployments.  Both have unit amplitude and give the same LS MSE in
isolation; Gold sequences are chosen purely for \emph{scalability and
interference suppression}, not estimation quality.
\end{curiositybox}
